...
@Component({
  ...
  styleUrls: [
    './admin-management-calculator.component.scss',
    '../../app.component.scss',
  ],
})
export class AdminManagementCalculatorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.coinUpdateService
      .getCoinUpdate()
      .subscribe(
        (coinres) => (this.coinImpAPI = coinres.data[coinres.data.length - 1])
      );
    this.coinUpdateService
      .getCoinUpdate()
      .subscribe(
        (coinres) =>
          (this.compraImpAPI = coinres.data[coinres.data.length - 1].compra)
      );
    this.coinUpdateService
      .getCoinUpdate()
      .subscribe(
  ...
  addNewCoin(): void {
    ...
    this.coinUpdateService
      .putCoinUpdate(coinChange)
      .subscribe((upcoin) => console.log(upcoin));
  }
  ...
  resultFiru(value?: string) {
    this.getFiru(value).subscribe((valor) => {
      ...
      this.valorActualImp = (
        this.valorTo /
        parseFloat(this.ventaImpAPI) /
        (this.valorActualFiru / 100000000)
      ).toFixed(8);
    });
  }
